...
// @version      2025-07-05
// @description  Userscript para o site https://animefire.io/
// @author       Mikill (discord: mikill9150)
// @match        https://animefire.io/*
...
(function() {
    ...
    async function checkVersion() {
        // Verificação de versão desabilitada - pop-up removido
        return;
        /*
        try {
    ...
    function showUpdateAlert() {
        // Função desabilitada - pop-up removido
        return;
        /*
        showModal(
...
<div class="block-container">
  ...
  <div class="input-container">
    <input id="userUrl" class="block-input" type="text" placeholder="https://animefire.io/users/988233449">
...
const aplicarImagensFundo = () => {
  if (window.location.href.startsWith('https://animefire.io/verify')) return;
...
if (originalBtn) {
  ...
  autoBtn.addEventListener('click', async () => {
    ...
    try {
      const res = await fetch('https://animefire.io/verify/index');
      const html = await res.text();
      const parser = new DOMParser();
      const doc = parser.parseFromString(html, 'text/html');
      const tokenInput = doc.querySelector('input[name="csrf_token"]');
      if (!tokenInput) {
        location.href = 'https://animefire.io/';
        return;
      }
      ...
      await fetch('https://animefire.io/auth/entrar', {
        method: 'POST',
        body: formData,
        redirect: 'manual'
      });
      ...
      if (loginAttempted) location.href = 'https://animefire.io/';
    }
  });
...
document.querySelector('form[action="https://animefire.io/proc/delete_historic"] button[type="submit"]').addEventListener('click', function(e) {
    e.preventDefault();
...
(function() {
    ...
    if (window.location.href.includes('/animes/')) {
        const targetProfiles = [
            'https://animefire.io/users/988233449',
            'https://animefire.io/users/460906716'
        ];
        ...
        const processComment = (comment) => {
            ...
            if (likeImg && likeImg.src.includes('/like.png')) {
                ...
                fetch('https://animefire.io/proc/cmt', {
                    method: 'POST',
                    headers: {
...
const checkAndAddDownloadButtons = () => {
  ...
  async function startDownloads(quality, seasonIndex, episodes) {
    ...
    for (const [index, episode] of episodes.entries()) {
      ...
      const downloadPageUrl = `https://animefire.io/download/${animeSlug}/${episodeNum}`;
...
(function() {
    ...
    if (config.divulgar === 'on' && window.location.href.includes('https://animefire.io/users/')) {
        ...
        if (form) {
            ...
            if (novaBio.length <= 350) {
                ...
                fetch('https://animefire.io/edit/update_data', {
                    method: 'POST',
                    body: formData,
                    credentials: 'include'
                });
...
(function() {
    ...
    function getExistingNotifications() {
      ...
      return Array.from(container.querySelectorAll('a.dropdown-item')).map(item => {
        const link = item.getAttribute('href').replace('https://animefire.io', '');
    ...
    function createNotificationElement(notification) {
      ...
      element.href = `https://animefire.io${notification.link}`;
      ...
      element.innerHTML = `
        <div class="d-flex" data-tema-processado="true">
          <div style="width:115px" data-tema-processado="true">
            <img class="img_cmt_noti mr-2" src="https://animefire.io${notification.reply_avatar}" data-tema-processado="true">
          </div>
          <div class="spanTextNT" style="width:100%;display:flex;flex-direction:column;justify-content:space-between" data-tema-processado="true">
            ...
            <div class="d-flex justify-content-between" style="margin-bottom:-3px" data-tema-processado="true">
              <span class="span_icon_lk_noti" data-tema-processado="true">
                <img class="mr-1" src="https://animefire.io/img/icons/link.png" data-tema-processado="true">
      ...
      const content = `
        <div class="notification-content">
          <div style="display:flex;align-items:center;gap:15px;margin-bottom:15px;">
            <img src="https://animefire.io${notification.reply_avatar}" style="width:50px;height:50px;border-radius:50%;border:2px solid ${themeColor};">
            <div>
          ...
          <div style="margin-top:15px;font-size:0.8em;display:flex;align-items:center;gap:5px;">
            <img src="https://animefire.io/img/icons/link.png" style="width:12px;">
            ${notification.slug}
          </div>
        </div>
      `;

      showModal('Nova Mensagem', content, `https://animefire.io${notification.link}`);
    }
...
if (matchResult && shouldRunAllSeasons()) {
    ...
    const checkSeasonExists = async (season, prefix = '') => {
        ...
        const seasonUrl = `https://animefire.io/animes/${baseName}${prefix}${dubbedSuffix}-todos-os-episodios`.replace(/--+/g, '-');
        try {
...
const getEpisodesList = async () => {
  const response = await fetch('https://animefire.io/');
  const html = await response.text();
  const parser = new DOMParser();
  const doc = parser.parseFromString(html, 'text/html');
...
const updateLocalStorageEpisodes = async () => {
  ...
  if (window.location.href === 'https://animefire.io/') {
